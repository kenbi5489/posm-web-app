...
const ListView = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        // Fetch all items from DB for filter derivation (Weeks/Brands)
        const allDbItems = await db.posmData.toArray();
        setAllItems(allDbItems);
        
        // Apply PIC filter if necessary for the actual list
        let filteredByPic = allDbItems;
        if (picId) {
          const pidNorm = String(picId).trim().toLowerCase();
          const pName = stripAccents(user?.ho_ten || selectedStaff?.ho_ten || '').toLowerCase().trim();
          
          filteredByPic = allDbItems.filter(i => {
  ...
  const { filteredItems, uniqueWeeks, uniqueBrands } = useMemo(() => {
    ...
    // 2. Filter for DONE + Filters (From items visible to user)
    let filtered = listItems.filter(i => i.status?.toLowerCase() === 'done');
  ...
  }, [allItems, listItems, week, search, selectedBrand]);
            ...
            {districtGroups.map((group, gIdx) => (
              <motion.section key={group.name} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: gIdx * 0.05 }} className="space-y-6">
                <div className="flex items-center justify-between px-2">
                   ...
                   <span className="bg-slate-100/50 text-slate-400 text-[10px] font-black px-4 py-1 rounded-full">{group.done} báo cáo</span>
                </div>
