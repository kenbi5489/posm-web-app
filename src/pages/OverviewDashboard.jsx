import {
  AlertTriangle, ChevronDown, Clock, RefreshCw, Trophy, Users, ShieldAlert, BadgeInfo, MapPin as MapPinIcon
} from 'lucide-react';
import { Link } from 'react-router-dom';
import { useSync } from '../hooks/useSync';
import { getCustomWeekNumber, getCurrentWeekLabel } from '../utils/weekUtils';
...
const OverviewDashboard = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        let adhocArray = [];
        try {
          if (db.adhocPoints) adhocArray = await db.adhocPoints.toArray();
        } catch (adhocErr) {
          console.warn("Failed to load adhocPoints:", adhocErr);
        }
  ...
  const { filtered, filteredAdhoc, uniqueWeeks, uniqueBrands } = useMemo(() => {
    ...
    const baseAdhoc = activeStaff ? adhocData.filter(i => {
      const mPId = String(i.pic_id || '').trim().toLowerCase();
      const mPName = stripAccents(i.pic || '').toLowerCase().trim();
      return (pidNorm && mPId && mPId === pidNorm) || (pNameNorm && mPName && mPName === pNameNorm);
    }) : adhocData;
    ...
    const syncedAdhocs = filteredData.filter(i => String(i.job_code || '').toUpperCase().includes('NEW_'));
    ...
    const adhocFiltered = adhocData.filter(i => {
      ...
      const mPId = String(i.pic_id || '').trim().toLowerCase();
      const mPName = stripAccents(i.pic || '').toLowerCase().trim();
      const picMatch = (pidNorm && mPId && mPId === pidNorm) || (pNameNorm && mPName && mPName === pNameNorm);
      
      return okWeek && okBrand && picMatch;
    });

    return { filtered: f, syncedAdhocs, filteredAdhoc: adhocFiltered, uniqueWeeks: uWeeks, uniqueBrands: uBrands };
  ...
  const kpis = useMemo(() => {
    ...
    filteredAdhoc.forEach(item => { if (item.job_code) adhocMap.set(item.job_code, item); });
    const finalAdhoc = Array.from(adhocMap.values());

    const assignedTotal = filtered.length;
    const assignedDone = filtered.filter(i => i.status?.toLowerCase() === 'done').length;

    // Include ad-hoc points in the total done count
    const adhocDone = finalAdhoc.length; // adhoc entries in DB are by definition 'done'
    const totalDone = assignedDone + adhocDone;

    // Tỉ lệ hoàn thành: 75 points = 100% (Static target from user logic)
  ...
  }, [filtered, syncedAdhocs, filteredAdhoc]);
  ...
  const chartData = useMemo(() => {
    ...
    // Ad-hoc points (always done)
    filteredAdhoc.forEach(item => {
      const w = item.week || 'N/A';
      if (!map[w]) map[w] = { total: 0, done: 0 };
      map[w].total++;
      map[w].done++;
