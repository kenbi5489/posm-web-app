...
function doPost(e) {
  try {
    ...
    // Tìm column index cho từng field bằng cách scan trực tiếp (không normalize phức tạp)
    const colIdx = {};
    const targetFields = [
      "Tên nhân viên", "Brand", "Địa chỉ", "Mã cv",
      "Project", "Hoạt động UrGift", "Ghi chú",
      "Link 1", "Link 2",
      "Mall_Name", "Location_Type", "District", "City",
      "POSM_Status", "Frame", "Lý do không có POSM"
    ];
    headerRow.forEach(function(h, i) {
      const hClean = String(h || "").trim();
      targetFields.forEach(function(f) {
        if (hClean === f && !colIdx[f]) {
          colIdx[f] = i; // 0-based index
        }
      });
    });
